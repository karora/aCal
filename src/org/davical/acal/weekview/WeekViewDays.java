...
package org.davical.acal.weekview;
...
public class WeekViewDays extends ImageView implements OnTouchListener {
    ...
    private void calculateHeaderVars() {
        ...
        HST = this.context.getCurrentDate().getEpoch() - ((long) scrollx * HSPP);
        HET = HST + HNS;
    ...
    public void drawGrid(Canvas canvas, Paint p) {
        int scrollx = context.getScrollX();
        p.setStyle(Paint.Style.FILL);
        int dayX = (int) (-WeekViewActivity.DAY_WIDTH + scrollx);
    ...
    private void drawEvents(Canvas canvas, Paint p) {
        ...
        int dayX = (int) (-WeekViewActivity.DAY_WIDTH + context.getScrollX());
    ...
    public void drawVertical(WVCacheObject event, Canvas canvas, int x, int width, long dayStart) {
        ...
        if (width < 1f) {
            if (Constants.LOG_VERBOSE && Constants.debugWeekView)
                Log.v(TAG, "Event is width " + width);
            return;
        }
        ...
        if (bottom < PxH) {  // Event is off top
            if (Constants.LOG_VERBOSE && Constants.debugWeekView)
                Log.v(TAG, "Event is off top by " + bottom + " vs. " + PxH);
            return;
        }
        ...
        if (top > TpX) { // Event is off bottom
            if (Constants.LOG_VERBOSE && Constants.debugWeekView)
                Log.v(TAG, "Event is off bottom by " + top + " vs. " + TpX);
            return;
        }
    ...
    public void drawHorizontal(WVCacheObject event, Canvas c, int depth) {
        ...
        int x = (int) (event.getStart() / 1000 - HST) / HSPP;
        if (x < 0) x = 0;
        int y = HIH * depth;
        int actualWidth = (int) Math.min(Math.min(event.getActualWidth(), viewWidth - x), (event.getEnd() / 1000 - HST) / HSPP);
